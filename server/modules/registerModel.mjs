...
const registerModel = {
    ...
    async (username, email, password, role) => {
        const newUser = await pool.query(`INSERT INTO users (user_name, email) VALUES ($1, $2);`, [username, email]);
        ...
        const findUserId = await pool.query('SELECT * FROM users WHERE email = $1',[email]);
            ...
            const savePassword = await pool.query('INSERT INTO users_secrets (userid, password, roleid ) VALUES ($1, $2, $3);',[userId, password, role ]);
